...
class person_follower(Node):
    """
    A ros node to make the Neato follow object and 
        actively looks for an object if there is no object in view range
    ...
    When no object in view range, 
        go at random angle and speed to look for objects to follow.
    ...
    def calculate_center(self):
        ...
        if cnt == 0:
            return 0,0,0
        return cnt, sum_delta_x/cnt, sum_delta_y/cnt
    ...
    def send_msg(self):
        ...
        if not self.bumper_active:
            ...
            cnt,dx,dy = self.calculate_center()
            if cnt == 0:
                self.wander = True
                self.approach = False
            else:
                ...
                self.wander = False
            if self.wander:
                # When no object is found, turn to a speed.
                print('wandering around!')
                z_spd = random()*self.kp 
                sign = 1 if random() < 0.5 else -1
                msg.angular.z = z_spd * sign
                msg.linear.x = random()*self.kp
            elif self.approach:
